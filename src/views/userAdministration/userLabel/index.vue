  ...
  <PageWrapper contentFullHeight contentBackground>
    <BasicTable @register="registerTable">
      ...
      <template #label="{ record }">
        <!-- <div
          class="px-2 py-[1px] bg-[#F0F5FF] border-1 border-[#ADC6FF] text-[#2F54EB] rounded-[2px] leading-[20px]"
        >
          {{ record.label }}
        </div> -->
      ...
      <template #status="{ record }">
        <a-button type="link" @click="openSettingModal(true, { record })" color="processing"
          >编辑</a-button
        >
...
<script lang="ts" setup name="ExportRecord">
  ...
  import addUserLabelModal from './components/addUserLabelModal.vue';
  ...
  const categoryKey = { all: '不限', law: '法律咨询师', general: '普通咨询师' };
